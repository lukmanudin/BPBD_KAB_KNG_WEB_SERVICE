        ...
        <div class="col-lg-12" id="notifikasi_bar">  
          ...
          <div class="table-responsive">
            <table class="table table-striped table-consensed">
              ...
              <tbody id="tabel_data">
                <tr>
                  <td colspan="5">Tidak ada peringatan dini tersimpan di database!</td>
                </tr>
              </tbody>
              <tfoot>
                <tr>
            ...
            $(document).ready(function(){
              $.ajax({
                url: "<?php echo site_url(); ?>/api/peringatan_dini/",
                type: 'GET',
            ...
            function edit_data(id){
              window.location = "<?php echo site_url(); ?>/apps/peringatan_dini/edit/" + id + "/";
            }

            function delete_data(id){
              var del = confirm("Apakah Anda yakin akan menghapus peringatan dini ?");
              if(del){
                $.ajax({
                    url: "<?php echo site_url(); ?>/api/peringatan_dini_delete/" + id + "/",
                    type: 'GET',
                    dataType : 'json',
                    success: function(response) {
                    ...
                    error: function(response){
                      alert("Delete data peringatan dini gagal.");
                    },
                    cache: false,
                    contentType: false,
                    processData: false
            ...
            function detail(id){
              window.open('<?php echo site_url("apps/mobile_peringatan_dini_detail/'+ id +'") ?>','Detail Peringatan Dini','directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=800,height=300');
            }
            </script>
          </div>
        </div>
